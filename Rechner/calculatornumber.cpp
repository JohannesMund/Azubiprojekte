...
CalculatorNumber::CalculatorNumber(const CalculatorNumber& c)
    : QObject(0)
{
    *this = c;
}
...
void CalculatorNumber::setVolatile() { _volatile = true; }
...
{
    return (op._integerPart == _integerPart && op._floatPart == _floatPart && op._floatLeadingZeroes == _floatLeadingZeroes && op._negated == _negated);
}
...
bool CalculatorNumber::operator>(const double op) const { return get() > op; }
...
void CalculatorNumber::operator=(const double& op) { set(op); }
