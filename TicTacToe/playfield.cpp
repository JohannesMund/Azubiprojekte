...
bool PlayField::setField(const int x, const int y,
...
void PlayField::checkWinner() {
  ...
  for (int col = 0; col <= 2; col++) {
    if (_grid[0][col] == _grid[1][col] && _grid[0][col] == _grid[1][col] &&
  ...
  if ((_grid[0][0] == _grid[1][1] && _grid[0][0] == _grid[2][2]) &&
      _grid[0][0] != PlayerManagement::none) {
    haveWinner(_grid[0][0]);
    return;
  }

  if ((_grid[0][2] == _grid[1][1] && _grid[0][2] == _grid[2][0]) &&
      _grid[0][2] != PlayerManagement::none) {
...
bool PlayField::gameOver() const { return _gameOver; }

PlayerManagement::Player PlayField::winner() const { return _winner; }
