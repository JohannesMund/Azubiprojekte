...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->cbModeSelect->insertItem(static_cast<int>(GameMode::pvcEasy),
                               "Computer (leicht)");
  ui->cbModeSelect->insertItem(static_cast<int>(GameMode::pvcMedium),
                               "Computer (medium)");
  ui->cbModeSelect->insertItem(static_cast<int>(GameMode::pvcHard),
                               "Computer (schwer)");
...
void MainWindow::buttonPressed(const int x, const int y) {
  ...
  if (_grid.setField(PlayField::PlayFieldCoords(x, y),
                     _playerManagement.getCurrentPlayer())) {
...
void MainWindow::newTurn() {
  ...
  if (_playerManagement.isPlayerTurn()) {
    ui->lbOut->setText(QString("Spieler %1 ist dran")
                           .arg(_playerManagement.currentPlayerName()));
  } else {

    PlayField::PlayFieldCoords coords;

    if (_currentMode == GameMode::pvcMedium ||
        _currentMode == GameMode::pvcHard) {
...
void MainWindow::on_pb1_clicked() { buttonPressed(0, 0); }

void MainWindow::on_pb2_clicked() { buttonPressed(0, 1); }

void MainWindow::on_pb3_clicked() { buttonPressed(0, 2); }

void MainWindow::on_pb4_clicked() { buttonPressed(1, 0); }

void MainWindow::on_pb5_clicked() { buttonPressed(1, 1); }

void MainWindow::on_pb6_clicked() { buttonPressed(1, 2); }

void MainWindow::on_pb7_clicked() { buttonPressed(2, 0); }

void MainWindow::on_pb8_clicked() { buttonPressed(2, 1); }

void MainWindow::on_pb9_clicked() { buttonPressed(2, 2); }
